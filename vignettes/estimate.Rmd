...
We now construct the `ctsmrTMB` model object as per usual
...
2. `method='ukf'`: The Unscented Kalman Filter.
3. `method='tmb'`: The Laplace Approximation
...
The latter `tmb` method is aptly named after the `TMB` package which employs the Laplace Approximation method to integration out random effects. In this formulation we consider the states as random effects, and parameters as fixed. The underlying assumption is one of normality, but its implementation allows for the flexibility of choosing arbitrary distributions (not yet implemented.)

Both of these methods can be used to estimate parameter and states. In the case of `tmb` the states will be smoothed (conditioned on both past and future observations).
