...
We initialise a `sdem` model object using
```{r}
library(sdem)
obj = sdem$new()
```
...
We can display a standard residual analysis for the observations by calling `plot` on the `fit` object, which invokes the `S3` `plot.sdem.fit` method of `plot`. This includes a quantile-quantile plot, histogram, auto-correlations and cumulative periodogram.
