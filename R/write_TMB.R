...
\treturn R_IsNA(asDouble(x));
  ...
  temptxt = append(temptxt, sprintf("\tmatrix<Type> G(%i,%i);",n,ng))
  for(i in 1:n){
    for(j in 1:ng){
...
{")
  ...
  txt = append(txt,sprintf("\tint __ng = %s;",ng))
  # txt = append(txt,sprintf("\tint __m = %s;",m))
  txt = append(txt,"\tType __nll = 0;")
  writeLines(txt,full_modelname)
  
  # Create variables
  txt = append(txt, "\tvector<Type> __f(__n);")
  txt = append(txt, "\tvector<Type> __Xi(__n);")
  txt = append(txt, "\tvector<Type> __Xip1(__n);")
  txt = append(txt, "\tvector<Type> __Z(__n);")
  txt = append(txt, "\tmatrix<Type> __G(__n,__ng);")
  txt = append(txt, "\tmatrix<Type> __V(__n,__n);")
  txt = append(txt, "\tmatrix<Type> __I(__n,__n);")
  txt = append(txt, "\t__I.setIdentity();")
  txt = append(txt, "\t__I *= 1e-8;")
  writeLines(txt,full_modelname)
  ...
  txt = append(txt, sprintf("\t\t__V = (__G*__G.transpose() + __I) * __dt;"))
  ...
  # Write observation contribution to likelihood - using NA(data) approach like with kalman filter
  for(i in 1:m){
