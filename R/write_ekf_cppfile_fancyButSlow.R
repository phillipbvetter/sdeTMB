write_ekf_cppfile_fancyButSlow = function(self, private) {
...
struct LONV_t : vector<vector<Type>> {
  ...
  if(length(private$input.names)>0){
    for(i in 1:length(private$input.names)){
      fvars2 = sub(pattern=sprintf("^%s$",private$input.names[i]), replacement=sprintf("%s(ii)",private$input.names[i]), x=fvars2)
    }
  }
  ...
  if(length(private$input.names)>0){
    for(i in 1:length(private$input.names)){
      dfdxvars2 = sub(pattern=sprintf("^%s$",private$input.names[i]), replacement=sprintf("%s(ii)",private$input.names[i]), x=dfdxvars2)
    }
  }
  for(i in 1:private$n){
  ...
  if(length(private$input.names)>0){
    for(i in 1:length(private$input.names)){
      gvars2 = sub(pattern=sprintf("^%s$",private$input.names[i]), replacement=sprintf("%s(ii)",private$input.names[i]), x=gvars2)
    }
  }
  for(i in 1:private$n){
  ...
  if(length(private$input.names)>0){
    for(i in 1:length(private$input.names)){
      hvars2 = sub(pattern=sprintf("^%s$",private$input.names[i]), replacement=sprintf("%s(ii)",private$input.names[i]), x=hvars2)
    }
  }
  for(i in 1:private$n){
  ...
  if(length(private$input.names)>0){
    for(i in 1:length(private$input.names)){
      dhdxvars2 = sub(pattern=sprintf("^%s$",private$input.names[i]), replacement=sprintf("%s(ii)",private$input.names[i]), x=dhdxvars2)
    }
  }
  for(i in 1:private$n){
  ...
  if(length(private$input.names)>0){
    for(i in 1:length(private$input.names)){
      obsvars2 = sub(pattern=sprintf("^%s$",private$input.names[i]), replacement=sprintf("%s(ii)",private$input.names[i]), x=obsvars2)
    }
  }
  for(i in 1:private$n){
  ...
  # Lists of Permutation Matrices and Data
  txt = c(txt, "\t DATA_STRUCT(list_of_permutation_mats, LOIM_t);")
  txt = c(txt, "\t DATA_STRUCT(list_of_data_vectors, LONV_t);")
  txt = c(txt, "\t DATA_VECTOR(length_of_data);")
  ...
  txt = c(txt, "\t int ii = 0;")
  txt = c(txt, "\t Type s__, log2PI = log(2*M_PI), nll__ = 0;")
  txt = c(txt, "\t vector<Type> e__,y__,F__,H__;")
  txt = c(txt, "\t matrix<Type> C__,R__,K__,E__,C2__,V2__,Ri__,A__,G__;")
  ...
  # Time for-loop
  txt = c(txt, "\t for(int i=0 ; i < length_of_data.size()-1 ; i++){")
  # txt = c(txt, "\t for(int i=0 ; i < 12 ; i++){")
  ...
  # Data Update
  txt = c(txt, "\n\t\t //////////// data-update ///////////")
  txt = c(txt, "\t\t y__  = list_of_data_vectors(i+1);")
  txt = c(txt, "\t\t s__  = length_of_data(i+1);")
  txt = c(txt, "\t\t E__  = list_of_permutation_mats(i+1);")
  ...
  txt = c(txt, "\t\t nll__ += Type(0.5) * atomic::logdet(R__) + Type(0.5) * lossfunction__((e__ * (Ri__ * e__)).sum(), tukey_pars__, loss_c_value__, which_loss__) + Type(0.5) * log2PI * s__;")
  txt = c(txt, "\n")
  ...
  txt = c(txt, "\t REPORT(list_of_data_vectors);")
